...
/* Mixer control names */
/* RvdB names changed */
#define MIXER_PCM_PLAYBACK_VOLUME           "Master Playback Volume"
...
#define MIXER_MIC_CAPTURE_VOLUME            "Capture Volume" 
...
#define MIXER_MIC_CAPTURE_SWITCH            "Capture Switch"
...
#define ANDROID_SAMPLING_RATE 44100

/* sampling rate when using SPDIF port */
#define MM_SPDIF_SAMPLING_RATE 44100
...
/* Values changed RvdB */
#define PERC_TO_PCM_VOLUME(x)     ( (int)((x) * 87 )) 
#define PERC_TO_CAPTURE_VOLUME(x) ( (int)((x) * 31 )) 
#define PERC_TO_HEADSET_VOLUME(x) ( (int)((x) * 87 )) 
#define PERC_TO_SPEAKER_VOLUME(x) ( (int)((x) * 87 )) 
...
static int start_output_stream(struct shuttle_stream_out *out)
{
    ...
    //RvdB out->pcm = pcm_open(card, port, PCM_OUT | PCM_MMAP | PCM_NOIRQ, &out->config);
                         ...
                         size_t bytes)
{
    ...
    //RvdB ret = pcm_mmap_write(out->pcm, (void *)buf, out_frames * frame_size);
