...
#include <linux/delay.h> //RB
#include <linux/earlysuspend.h> //RvdB
...
/* RvdB pega_acc_axis rewritten:
 * the "normal" apci method's are "not working as expected" :-(
 * So I now use "direct" Embedded processor calls.
 * Much less timeouts this way !!
 * The lack of locking around the high en low bytes, is not solvable
 * (without rewriting the EC firmware).
 * So I asume that only one  transition in the High byte is possible
 * between 2 ec-calls. 
 * Since getting 1 byte takes about 9 ms, and changes in accelerometer
 * values only occur every 25-50 ms that should not be a problem
 * */
#define CMCM 0x04
#define CMD1 0x05
#define CMD2 0x06
#define CMD3 0x07
static int pega_acc_axis(u8 addr)
{
	int i,val;
	u8 t1hi,t2lo,t3hi ;
	  ...
	  t1hi = t3hi;
          // printk("RBa %08x %08x %08x \n",t1hi,t2lo,t3hi);
        }
        val = (short)((t1hi << 8) | t2lo) ;
	return clamp_val((short)val, -PEGA_ACC_CLAMP+1, PEGA_ACC_CLAMP-1);
}
...
{
	struct asus_laptop *asus ;
        printk("RvdB Stopping accelerometer\n");
	asus = container_of(handler, struct asus_laptop,early_suspend) ;
	asus->pega_accel_active = false ;
}
static void accel_early_resume(struct early_suspend *handler)
{
	struct asus_laptop *asus ;
        printk("RvdB Resume accelerometer\n");
	asus = container_of(handler, struct asus_laptop,early_suspend) ;
	asus->pega_accel_active = true ;
}
#endif
...
{
#ifdef CONFIG_HAS_EARLYSUSPEND
	if (asus->early_suspend.suspend)
          unregister_early_suspend(&asus->early_suspend);
#endif
}
...
{
#ifdef CONFIG_HAS_EARLYSUSPEND
	asus->early_suspend.suspend = accel_early_suspend;
        asus->early_suspend.resume  = accel_early_resume;
        register_early_suspend(&asus->early_suspend);
#endif
	asus->pega_accel_active = true ;
	return 0;
}
...
static void asus_acpi_notify(struct acpi_device *device, u32 event)
{
	...
	if (event == 0XEA && asus->pega_accel_active ) {
	  int x,y,z,orient ;
	  printk("RvdB Orientation change received\n");
	  x = pega_acc_axis(0x70);
	  y = pega_acc_axis(0x72);
	  z = pega_acc_axis(0x74);
	  if ( abs(x) > abs(y) ) {
	    orient = x < 0 ? KEY_F12 : KEY_F11 ;
	  } else {
	    orient = y < 0 ? KEY_F9 : KEY_F10 ;
	  }
	  if (asus->inputdev) {
	    input_report_key( asus->inputdev,orient,1) ;
	    input_sync(  asus->inputdev) ;
	    input_report_key( asus->inputdev,orient,0) ;
	    input_sync(  asus->inputdev) ;
	  }
	}
}
...
static int __devinit asus_acpi_init(struct asus_laptop *asus)
{
	...
	/* RvdB: Move next to a place where is_pega_lucid is defined !! 
