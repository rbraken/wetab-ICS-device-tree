...
#define MT_CLS_DUAL_CONFIDENCE_CONTACTID	0x0004
...
static void mt_emit_event(struct mt_device *td, struct input_dev *input)
{
	...
	for (i = 0; i < td->maxcontacts; ++i) {
		...
		struct mt_slot *so = &(td->slots[1-i]); //RvdB the other finger
		...
		if (s->touch_state) {
			...
			/* RvdB also export the other finger if on screen */
			if (so->touch_state) {
			  /* other finger also on screen */
			  input_mt_sync(input);
			  wide = (so->w > so->h);
			  major = max(so->w, so->h) >> 1;
			  minor = min(so->w, so->h) >> 1;
		          input_event(input, EV_ABS, ABS_MT_TOUCH_MAJOR, so->touch_state);
			  input_event(input, EV_ABS, ABS_MT_TRACKING_ID, so->contactid);
	  		  input_event(input, EV_ABS, ABS_MT_POSITION_X, so->x);
			  input_event(input, EV_ABS, ABS_MT_POSITION_Y, so->y);
			  input_event(input, EV_ABS, ABS_MT_ORIENTATION, wide);
			if (so->p == 0) so->p = 100 ; //RvdB firmware fix
			  input_event(input, EV_ABS, ABS_MT_PRESSURE, so->p);
			  input_event(input, EV_ABS, ABS_MT_WIDTH_MAJOR, major);
			  input_event(input, EV_ABS, ABS_MT_WIDTH_MINOR, minor);
			} /* RvdB */
		}
